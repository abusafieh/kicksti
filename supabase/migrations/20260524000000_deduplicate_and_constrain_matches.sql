/*
  # Deduplicate matches and add unique constraint

  1. Problem
    - Two seed migrations both inserted 72 matches without a unique constraint,
      resulting in 144 rows (every match duplicated).

  2. Changes
    - Clear all dependent data and matches
    - Re-insert the correct 72 group-stage fixtures
    - Add UNIQUE constraint on (home_team, away_team) to prevent future duplicates
*/
...
ALTER TABLE matches DROP CONSTRAINT IF EXISTS matches_home_away_unique;
ALTER TABLE matches ADD CONSTRAINT matches_home_away_unique UNIQUE (home_team, away_team);
